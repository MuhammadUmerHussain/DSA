...
int main()
{
	...
	if(input==NULL)//error if not open
	{
		printf("Error!");
		exit(1);
	}
	...
	while(getline(input,array))//reading line by line
	{
		...
		//cout<<i<<endl;
		if(array[i]>='0' && array[i]<='9')//if number are two digit then this condition would apply
		{
			...
			{	check=array[i]-'0';//single digit printing
				if(array[i-1]=='-' && array[i]>='1' && array[i]<='9' )//according to question if it is less than 0 then print Z
				{
					cout<<chara[25];
					output<<chara[25];
				}
